export const GET_ERRORS = 'GET_ERRORS';
export const USER_LOADING = 'USER_LOADING';
export const SET_CURRENT_USER = 'SET_CURRENT_USER';
export const REGISTER_SUCCESS = 'REGISTER_SUCCESS';
export const REGISTER_FAIL = 'REGISTER_FAIL';
export const USER_LOADED = 'USER_LOADED';
export const AUTH_ERROR = 'AUTH_ERROR';
export const LOGIN_SUCCESS = 'LOGIN_SUCCESS';
export const LOGIN_FAIL = 'LOGIN_FAIL';
export const LOGOUT = 'LOGOUT';
export const TOGGLE_SIDEBAR = 'TOGGLE_SIDEBAR';
export const ADD_ACCOUNT = 'ADD_ACCOUNT';
export const DELETE_ACCOUNT = 'DELETE_ACCOUNT';
export const GET_ACCOUNTS = 'GET_ACCOUNTS';
export const ACCOUNTS_LOADING = 'ACCOUNTS_LOADING';
export const ACCOUNTS_FAILURE = 'ACCOUNTS_FAILURE';
export const GET_TRANSACTIONS = 'GET_TRANSACTIONS';
export const TRANSACTIONS_LOADING = 'TRANSACTIONS_LOADING';
export const TRANSACTIONS_FAILURE = 'TRANSACTIONS_FAILURE';
export const GET_LIABILITIES = 'GET_LIABILITIES';
export const LIABILITIES_LOADING = 'LIABILITIES_LOADING';
export const LIABILITIES_FAILURE = 'LIABILITIES_FAILURE';
export const GET_INVESTMENTS = 'GET_INVESTMENTS';
export const INVESTMENTS_LOADING = 'INVESTMENTS_LOADING';
export const INVESTMENTS_FAILURE = 'INVESTMENTS_FAILURE';
export const GET_BALANCES = 'GET_BALANCES';
export const UPDATE_BALANCE = 'UPDATE_BALANCE';
export const BALANCES_LOADING = 'BALANCES_LOADING';
export const BALANCES_FAILURE = 'BALANCES_FAILURE';
